...
- `api-gateway/` — FastAPI application plus Celery tasks. Key sub-packages:
  ...
  - `app/services/` (business logic, OCR helpers, scoring)
...
## DOCX Metrics Extraction & Gemini Fallback
...
- **Локальный OCR → Gemini Vision fallback**: сначала PaddleOCR + PP-Structure, затем при низкой уверенности (<0.8) или неполной таблице отправляем фрагмент изображения в Gemini Vision, требуя строгий JSON по схеме (см. prompt в `extraction-pipeline.md`). Любые ответы, выходящие за диапазон или схему, отклоняем и повторяем.
...
- **Очереди Celery**: `ocr` (извлечение), `normalize` (валидаторы, маппинг), `vision` (Gemini). При ошибках используем ретраи и DLQ; логи не содержат PII.
