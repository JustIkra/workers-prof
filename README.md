...
# GEMINI_API_KEYS=ваш_gemini_api_ключ_здесь

# Настройка для Docker (используйте имена сервисов):
POSTGRES_DSN=postgresql+asyncpg://app:app@postgres:5432/app
REDIS_URL=redis://redis:6379/0
RABBITMQ_URL=amqp://guest:guest@rabbitmq:5672//

# Опционально: включите VPN для доступа к Gemini API
# VPN_ENABLED=1
# VPN_TYPE=awg
...
```bash
# Соберите образы (первый раз или после изменений)
...
При первом запуске:
- PostgreSQL база данных будет создана автоматически
- Миграции Alembic будут применены автоматически при старте app контейнера
- Базовые данные (метрики, профессиональные деятельности) будут загружены автоматически в dev/test/ci режиме
- Все сервисы (postgres, redis, rabbitmq) проверяются через health checks перед запуском app и worker
...
# Ожидаемые сервисы:
# - workers-prof-app (healthy)
# - workers-prof-worker (healthy)
# - workers-prof-postgres (healthy)
# - workers-prof-redis (healthy)
# - workers-prof-rabbitmq (healthy)

# Проверьте health endpoint
curl http://localhost:9187/api/healthz
# Ожидаемый ответ: {"status":"ok","service":"api-gateway","version":"0.1.0","env":"prod"}

# Проверьте применение миграций
docker-compose exec app alembic current
# Должна быть показана текущая версия миграции

# Проверьте логи app контейнера (должны быть сообщения об успешном применении миграций)
docker-compose logs app | grep -i migration

# Проверьте логи worker контейнера
...
> **Примечание**: Если вы используете VPN (AWG), убедитесь, что в `.env` установлено:
> ```bash
> VPN_ENABLED=1
> VPN_TYPE=awg
> WG_CONFIG_PATH=/app/wireguard/awg0.conf
> ```
> Конфиг `wireguard_awg.conf` автоматически монтируется в контейнеры через `docker-compose.yml`.
